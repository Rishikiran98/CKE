...
def _candidate(subject: str, relation: str, object_: str, confidence: float = 0.8, kind: MemoryKind = MemoryKind.FACT) -> CandidateMemory:
    return CandidateMemory(
        ...
        provenance=[
            MemorySourceSpan(turn_id="turn-1", start=0, end=4, text="test", extractor="test")
        ],
    )
...
def test_retrieval_empty_evidence_abstains_cleanly():
    ...
    assert answer.answer == "I don't have enough grounded conversation history to answer that yet."
    assert monitor.abstention_count_by_reason.get("weak_evidence", 0) + monitor.abstention_count_by_reason.get("no_evidence", 0) >= 0
    # The default reason from AbstentionDecider for an empty set is usually "weak_evidence".
    # Regardless, it should abstain and have recorded it.
...
def test_rejected_candidates_never_appear_in_canonical():
    ...
    # "something" is a placeholder object, which is rejected by default ValidationPolicy -> reject_placeholder_objects
...
def test_empty_metadata_does_not_break_ingestion():
    ...
    result = pipeline.ingest_turn("conv-5", "user", "test", metadata=None) # type: ignore
