...
class QueryRouter:
    ...
    def detect_entities(self, query: str, candidate_entities: Iterable[str] | None = None):
    ...
    def classify_domain(self, query: str):
        ...
        if any(k in q for k in ["redis", "database", "sql", "nosql"]):
            domains.append("databases")

        if any(k in q for k in ["api", "service", "endpoint"]):
            domains.append("systems")

        if any(k in q for k in ["model", "llm", "embedding"]):
            domains.append("ml")
