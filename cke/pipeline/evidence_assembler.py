...
class EvidenceAssembler:
    ...
    def assemble(
        ...
        filtered_facts = self._filter_facts(deduped_facts, resolved_entities, query_plan)
        ...
        return ReasoningContext(
            ...
            trace_metadata={
                ...
                "facts_by_canonical_subject": subgraph.get("facts_by_canonical_subject", {}),
    ...
    def _filter_facts(
        ...
        canonical_names = [AliasRegistry.normalize(e.canonical_name) for e in resolved_entities if e.canonical_name]
        canonical_ids = [AliasRegistry.normalize(e.entity_id) for e in resolved_entities if e.entity_id]
        ...
        for fact in facts:
            ...
            subject_id = AliasRegistry.normalize(fact.statement.canonical_subject_id or "")
            object_id = AliasRegistry.normalize(fact.statement.canonical_object_id or "")
            ...
            relation_match = int(any(term == relation or term in relation for term in relation_terms))
            score = fact.retrieval_score + (0.2 * entity_match) + (0.15 * relation_match)
        ...
        if len(resolved_entities) >= 2:
            ...
            keep = list(selected)
            seen_entities = {
                AliasRegistry.normalize(f.statement.subject)
                for f in keep
            }
    ...
    def _relation_terms(self, query_plan: QueryPlan) -> set[str]:
        terms: set[str] = {AliasRegistry.normalize(v) for v in getattr(query_plan, "target_relations", []) if v}
    ...
    def _build_candidate_paths(
        ...
        for left in facts:
            for right in facts:
                if left is right:
                    continue
                if AliasRegistry.normalize(left.statement.object) == AliasRegistry.normalize(right.statement.subject):
    ...
    def _build_subgraph(self, facts: list[EvidenceFact]) -> dict[str, list[dict[str, str]]]:
        ...
        for fact in facts:
            ...
            canonical_subject = fact.statement.canonical_subject_id or fact.statement.subject
