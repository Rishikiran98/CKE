...
def test_llm_reasoner_falls_back_without_api_key():
    ...
    template_answer = TemplateReasoner().answer("What protocol does Redis use?", context)
    assert llm_reasoner.answer("What protocol does Redis use?", context) == template_answer
...
def test_template_reasoner_behavior_unchanged():
    ...
    context = [Statement("Redis", "uses", "RESP")]
    assert reasoner.answer("What protocol does Redis use?", context) == "Redis uses RESP protocol."
