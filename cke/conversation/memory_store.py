...
class ConversationMemoryStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._canonical_by_conversation: dict[str, list[CanonicalMemory]] = defaultdict(
            list
        )
    ...
    def store_canonical_memory(self, memory: CanonicalMemory) -> None:
        ...
        if (
            self.config.indexing.enable_graph_projection
            and memory.status.value == "accepted"
        ):
        ...
        overflow = (
            len(bucket) - self.config.retention.max_canonical_memories_per_conversation
        )
        if overflow > 0:
            del bucket[:overflow]
    ...
    def facts_for_conversation(self, conversation_id: str) -> list[Statement]:
        return [
            memory.as_statement()
            for memory in self.get_canonical_memories(conversation_id)
            if memory.status.value == "accepted"
        ]

    def evidence_lookup(
        self, conversation_id: str, memory_ids: list[str]
    ) -> EvidenceSet:
        memories = [
            memory
            for memory in self.get_canonical_memories(conversation_id)
    ...
    def all_conversation_ids(self) -> list[str]:
        return sorted(
            set(self._events_by_conversation) | set(self._canonical_by_conversation)
        )
